...
try:
    ...
    IA_ACTIVA = False
    print("⚠️ IA no cargada, funcionando en modo normal")
...
def calcular_costo_plato(insumos_str, df_insumos):
    try:
        ...
        precios = df_insumos[df_insumos['id_insumo'].isin(ids_lista)]['precio_unidad']
        return round(precios.sum(), 2)
    except:
        return 0.0

# --- IA FUNCION (NUEVA, NO ROMPE NADA) ---
...
def actualizar_perfil():
    try:
        ...
        # 🔥 AQUÍ VA (ANTES DE MODIFICAR VALORES)
        ...
        print("🔥 ERROR en actualizar_perfil:", e)
...
def recomendar():
    try:
        ...
        # 🔥 SOLO ESTO SE AÑADE (FILTRO DE ALERGIAS)
        ...
        aptos['costo_estimado'] = aptos['insumos_base_ids'].apply(lambda x: calcular_costo_plato(x, BD['INSUMOS']))
        ...
        return render_template('recomendacion.html', p=p_data, s=secundarias, momento=momento, presupuesto=presupuesto_user)
...
def registrar_consumo():
    try:
        ...
        if not id_elegido:
            return "Error: no se recibió id_comida", 400
        ...
        if datos_plato.empty:
            return f"Error: plato {id_elegido} no existe", 404
        ...
        if historial.empty:
            ...
            nuevo_id_reg = int(historial['id_registro'].max() + 1)
